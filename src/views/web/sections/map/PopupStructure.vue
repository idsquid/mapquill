...
<!-- fullpage-->
        ...
        <div class="html-audio-posts" v-else-if="audioPosts.length" style="max-height: 100%; overflow: scroll;">
          <div class="html-audio-post" v-for="(ap, i) in audioPosts" :key="'ap-'+i">
            <div class="title">{{ ap.audioTitle || 'Untitled' }}</div>
            <audio controls @loadstart="popup.update()">
              <source :src="ap.audioUrl">
            </audio>
          </div>
        </div>
